...
async function translateDescriptions(skills, locale) {
    ...
    const results = skills.map(s => {
        const key = cacheKey(s.description, locale);
        if (cache[key]) return cache[key];
        // If source language matches target, no translation needed
        if (s.description && isChinese(s.description) === (locale === 'zh')) return s.description;
        toTranslate.push(s.description);
        toTranslateIdx.push(skills.indexOf(s));
    ...
    if (toTranslate.length > 0) {
        ...
        if (translated) {
            for (let i = 0; i < toTranslate.length; i++) {
                ...
                const key = cacheKey(skills[idx].description, locale);
