...
async function translateDescriptions(skills, locale, llm) {
    ...
    // Small batches keep each response well under the output-token ceiling.
    if (toTranslate.length > 0) {
        const CHUNK = 3;
        let allTranslated = [];
        for (let start = 0; start < toTranslate.length; start += CHUNK) {
            const batch = toTranslate.slice(start, start + CHUNK);
            let translated = null;
            if (llm) {
                const text = await llmComplete(llm, translationPrompt(batch, locale));
                if (text) translated = parseJsonArray(text, batch.length);
            }
            if (!translated) translated = await translateBatchDirect(batch, locale);
            if (!translated) { allTranslated = null; break; }
        ...
        if (allTranslated) {
            for (let i = 0; i < toTranslate.length; i++) {
                ...
                cache[key] = allTranslated[i];
                results[idx] = allTranslated[i];
            }
            saveCache();
        }
    }
